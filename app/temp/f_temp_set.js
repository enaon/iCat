...
ew.face[0] = {
    ...
    page1: function(batt, id) {
        ...
        ew.UI.c.start(1, 1);
        //ew.UI.btn.c2l("main", "_2x3", 1, "AUTO", this.dataString[this.data.tempm], 15, this.data.tempm ? 4 : 1);
        ew.UI.btn.c2l("main", "_2x3", 1, "TOP", ew.apps.temp.state.def.topL, 15, 6);
        ew.UI.btn.c2l("main", "_2x3", 2, "BTM", ew.apps.temp.state.def.btmL, 15, 6);
        ...
        ew.UI.c.main._2x3 = (i) => {
            if (i == 3) {
                ...
                this.data.tempm++;
                if (3 < this.data.tempm) this.data.tempm=0;
                
                if (ew.def.face.info) ew.UI.btn.ntfy(1, 1.5, 0, "_bar", 6, "HRM MODE", txt[this.data.tempm], 0, 15);
                ew.UI.btn.c2l("main", "_2x3", 1, "AUTO", this.dataString[this.data.tempm], 15, this.data.tempm ? 4 : 6);
            }
            else if (i == 1) {
                ew.sys.buzz.nav(ew.sys.buzz.type.ok);
                ew.UI.btn.ntfy(1, 3, 0, "_bar", 6, "< TOP LIMMIT >", "", 15, 6, 1);
                ew.is.slide = 1;
                ew.sys.TC.val = { cur: ew.apps.temp.state.def.topL, dn: 25, up: 45, tmp: 0, fire: 0 };
                ew.UI.c.tcBar = (a, b, r) => {
                    let val = ew.apps.temp.state.def.topL;
                    ew.UI.btn.ntfy(0, 2, 1);
                    if (11 < r && ew.sys.TC.val.dn < val && val < ew.sys.TC.val.up) val = val + (a * (20 < r ? 10 : 5));
                    else val = b;
                    ew.sys.TC.val.cur = val;
                    ew.apps.temp.state.def.topL = val;
                    ew.UI.btn.c2l("main", "_2x3", 1, "TOP", ew.apps.temp.state.def.topL, 15, 6);
                };
            }
            else if (i == 2) {
                ...
                ew.is.slide = 1;
                ew.sys.TC.val = { cur: ew.apps.temp.state.def.btmL, dn: 0, up:20, tmp: 0, fire: 0 };
                ew.UI.c.tcBar = (a, b, r) => {
                    ...
                    ew.sys.TC.val.cur = val;
                    ew.apps.temp.state.def.btmL = val;
                    ew.UI.btn.c2l("main", "_2x3", 2, "BTM", ew.apps.temp.state.def.btmL, 15, 6);
                };
            }
            else if (i == 4) {
