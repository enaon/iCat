...
ew.sys.TC = {
			...
			if (ew.face.pageCurr >= 0) {
				...
				if (ew.is.slide) {
					if (180 < (((tp[5] & 0x0F) << 8) | tp[6])) {
						if (!ew.sys.TC.tid) {
							if (ew.sys.TC.dbg) console.log("TC: init bar start")
							ew.sys.TC.tid = setInterval(function() {
								ew.sys.TC.bar();
							}, 30);
						}
						if (tp[1] == 5) {
							if (ew.sys.TC.dbg) console.log("TC:  init bar main")
							ew.sys.buzz.nav(ew.sys.buzz.type.ok)
							let side = (g.getWidth() / 2 < tp[4]) ? 1 : 0;
							if (ew.sys.TC.val.reverce) side = 1 - side;
							side ? ew.sys.TC.val.cur++ : ew.sys.TC.val.cur--;

							if (ew.sys.TC.val.up < ew.sys.TC.val.cur) ew.sys.TC.val.cur = (ew.sys.TC.val.loop) ? ew.sys.TC.val.dn : ew.sys.TC.val.up;
							else if (ew.sys.TC.val.cur < ew.sys.TC.val.dn) ew.sys.TC.val.cur = (ew.sys.TC.val.loop) ? ew.sys.TC.val.up : ew.sys.TC.val.dn;
				...
				if (a != 5 && a != 12)
					ew.UI.nav[dir[a]](tp[4],  (((tp[5] & 0x0F) << 8) | tp[6]));
				else{
					ew.UI.c.xy(tp[4], (((tp[5] & 0x0F) << 8) | tp[6]), a == 12 ? 1 : 0);
					if (ew.UI.ntid && !ew.is.bar && (180 < (((tp[5] & 0x0F) << 8) | tp[6]))) { ew.sys.buzz.nav(ew.sys.buzz.type.ok);ew.UI.rtb(); return; }
	
				}
				if (ew.sys.TC.dbg) console.log("( this.x + (this.x / 10), this.y )", ew.sys.TC.x + (ew.sys.TC.x / 10), ew.sys.TC.y)
	...
	bar: function() {
		var tp = i2c.readFrom(0x15, 7);
		if (ew.sys.TC.dbg) print("touch bar:", tp);
		...
		if (ew.is.slide && 180 < (((tp[5] & 0x0F) << 8) | tp[6])) {
			if (tp[2]) {
				...
				if (this.x != tp[4]) {
					if (ew.sys.TC.dbg) console.log("TC:  bar main")
											this.rp = 0;
						if (this.tim) {
					...
					//let len = 200/(this.val.up-this.val.dn);
					let len = 20;
					...
					if (ew.sys.TC.dbg) console.log("TC:  bar else")
					this.bt = 1;
			...
			ew.sys.TC.st = 1;
			ew.sys.TC.tid = 0;
			if (ew.sys.TC.dbg) console.log("TC:  bar stop")
		}
	},
	stop: function() {
		"ram";
