// health face - movement

ew.UI.nav.next.replaceWith(() => {
    ew.sys.buzz.nav(ew.sys.buzz.type.ok);
    ew.apps.move.state.def.page++;
    if (3 < ew.apps.move.state.def.page) ew.apps.move.state.def.page = 1;
    ew.face[0].mode(ew.apps.move.state.def.page);

});
ew.UI.nav.back.replaceWith(() => {
    ew.sys.buzz.nav(ew.sys.buzz.type.ok);
    ew.apps.move.state.def.page--;
    if (ew.apps.move.state.def.page < 1) ew.apps.move.state.def.page = 3;
    ew.face[0].mode(ew.apps.move.state.def.page);
});
...
ew.face[0] = {
    data: {
        ...
        key: "movement", // current key for graph
        pos: 0, // current position
        maxMove: 10000, // default max move
        topL:ew.apps.move.state.def.topL, 
        btmL:ew.apps.move.state.def.btmL,
    },
    run: false,
    ...
    show: function() {
        if (!this.run) return;
        this.mode(ew.apps.move.state.def.page || 0);
    },
    ...
    mode: function(a) {
        ...
        if (!this.data["source" + a]) {
            require("health")[period](time, (entry) => {
                //if (entry && entry.movement && entry.movement > 0) {
                if (entry) {
    ...
    pHAv: function() {
        ...
        for (let offset = 23; offset >= 0; offset--) {
            ...
            if (counts[displayHour] > 0) {
                hourlyData.push({
                    movement: sums[displayHour] / counts[displayHour] | 0,
                    hr: displayHour,
                    min: 0,
                });
            }
    ...
    pHDa: function() {
        ...
        for (let i = 0; i < this.data.source.length; i++) {
            if (this.data.source[i].movement > this.data.maxMove) {
                this.data.maxMove = this.data.source[i].movement;
            }
        }
    ...
    dIn: function() {
        ...
        let moveStr = entry.movement.toString();
    ...
    dGr: function(update, newPos) {
        ...
        if (update) {
            ...
            let oldBarH = oldEntry.movement * scale;
            
            let color =  topL < oldEntry.movement ? 13 : oldEntry.movement < btmL? 9: 4;
            ...
            let newBarH = newEntry.movement * scale;
            ...
            for (let i = 0; i < fields; i++) {
                let entry = this.data.source[i];
                let barH = entry.movement * scale;
                let x = startX + i * (bw + space);
                let isSelected = (i === this.data.pos);

                let color =  topL < entry.movement ? 13 : entry.movement < btmL? 9: 4;
