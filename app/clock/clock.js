...
if (ew.def.bri<3) ew.UI.theme.current= ew.UI.theme.white;
ew.UI.theme.current= ew.UI.theme.dark;
ew.face[0] = {
	offms: (ew.def.off[ew.face.appCurr])?ew.def.off[ew.face.appCurr]:10000,
	old:ew.def.bpp?0:1,
	init: function(){
		//g.clear(1);
...
//			dateN:[0,55,140,65],
		...
		//else ew.UI.btn.ntfy(1,3,0,"_bar",6,ew.def.name,". . . . . . . . .",11,0,0);g.flip();
			...
			//ew.UI.btn.c2l("main","_2x3",3,ew.def.txt?"OFF":tm,ew.def.txt?tm:"",15,1);

			ew.UI.btn.c2l("bar","_bar",6,"eucWatch",ew.def.name,15,0,0);
	...
	bar:function(){
		...
		ew.UI.c.end();
		//print("oo");
		//ew.UI.btn.img("bar","_bar",1,"settings",0,14,0);
		//ew.UI.btn.img("bar","_bar",2,"watch",0,14,0);
		//ew.UI.btn.img("bar","_bar",3,"dash",0,14,0);
	...
	bat:function(){
			...
			if (this.v<=0) {g.setFont("Vector",this.gui.txt);g.drawString("EMPTY",this.gui.bat[2]-5-(g.stringWidth("EMPTY")),this.gui.bat[1]); 
				...
				g.drawImage((this.batt==1)?require("heatshrink").decompress(atob("jEYwIKHiACEnACHvACEv/AgH/AQcB/+AAQsAh4UBAQUOAQ8EAQgAEA==")):require("heatshrink").decompress(atob("jEYwIEBngCDg//4EGgFgggCZgv/ASUEAQQaBHYPgJYQ=")),this.gui.bat[0],this.gui.bat[1]);
