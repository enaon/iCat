...
ew.face[0] = {
    ...
    init: function() {
        ...
        if (!ew.apps.itag.state.connected&&!ew.apps.itag.state.ble.gatt.connected)
    ...
    show: function(o) {
        if (!ew.apps.itag.state.connected &&!ew.apps.itag.state.ble.gatt.connected) this.tid=setTimeout(()=>{ew.apps.itag.conn(ew.apps.itag.state.ble.id)},800);
    },
    getVal:function (val){
