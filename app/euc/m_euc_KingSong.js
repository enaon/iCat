...
ew.apps.euc.cmd = (no, val) => {
	//"ram";
	let euc = ew.apps.euc;
	if (euc.dbg) console.log("command send :", no, val);
	switch (no) {
		//euc.wri("getParamA");
		case "manual":
			return val;
...
ew.apps.euc.help.temp.resp = function(inpk) {
	...
	// "ram";
	if (2 < euc.dbg) console.log("id, responce:", inpk[16], inpk);
	if (inpk[16] == 63)
		euc.state.dash.auto.offT = inpk[5] << 8 | inpk[4];
	else if (inpk[16] == 67) {
		if (2 < euc.dbg) console.log("**** bt pass:", inpk);
		if (inpk[6] == 1) {
		...
		if (2 < euc.dbg) console.log("***** bt pass state:", inpk);
		...
		.then(function(g) {
			...
			euc.test = () => {
				...
				if (euc.state.status != "OFF") {
					euc.state.tout.buffer = setTimeout(() => {
						euc.test();
					}, 500);
				}
			...
			ew.apps.euc.wri = function(n, v) {
				let euc = ew.apps.euc;
				if (euc.dbg) console.log("EUC module Kingsong wri:", n, v);

				if (euc.state.tout.busy) {
					clearTimeout(euc.state.tout.busy);
					euc.state.tout.busy = setTimeout(() => { euc.state.tout.busy = 0; }, 70);
					return;
				}
				euc.state.tout.busy = setTimeout(() => { euc.state.tout.busy = 0; }, 100);
				if (n === "proxy") {
					if (euc.dbg) console.log("EUC module Kingsong proxy cmd:", v);
					...
					euc.state.is.horn = 1;
					if (euc.state.tout.horn) {
						clearTimeout(euc.state.tout.horn);
						euc.state.tout.horn = 0;
					}
					...
					if (euc.dbg) print("EUC module Kingsong Start sequence");

					euc.state.status = "READY";
					c.startNotifications().then(function() {
						...
						if (euc.dbg) print("EUC module Kingsong end write sequence");

						if (!euc.state.dash.info.get.serl) return c.writeValue(euc.cmd("getSerial"));
					}).catch(euc.off);
				}
				else if (euc.state.status == "OFF" || n == "end") {
					if (euc.dbg) print("EUC module Kingsong start off state sequence");
			...
			*/
			if (euc.dbg) print("EUC module Kingsong starting");
			ew.sys.buzz.nav([90, 40, 150]);
			euc.wri("start");
			//}
			//reconect
		}).catch(euc.off);
